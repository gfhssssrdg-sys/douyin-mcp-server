...
2. 下载视频并提取音频
3. 使用 DashScope 或火山引擎 / 火山方舟 ASR 从音频中提取文本
...
class DouyinProcessor:
    ...
    def __init__(
        ...
        self.api_base_url = api_base_url or self._default_api_base_url(self.provider)
    ...
    def _default_api_base_url(provider: str) -> str:
        if provider == "volcengine":
            return f"{DEFAULT_ARK_BASE_URL}/audio/transcriptions"
    ...
    def transcribe_single_audio(self, audio_path: Path) -> str:
        ...
        try:
            ...
            response = requests.post(self.api_base_url, files=files, data=data, headers=headers, timeout=300)
            response.raise_for_status()
            result = response.json()
            return result.get("text") or result.get("result") or result.get("transcript") or response.text
        except Exception as e:
    ...
    def extract_text_from_audio(self, audio_path: Path, show_progress: bool = True) -> str:
        ...
        if not self.api_key:
            raise ValueError("未设置 API 密钥，请检查 provider 对应的环境变量")
        ...
        if not need_split:
            if show_progress:
                print(f"正在使用 {self.provider} 识别语音...")
...
def resolve_asr_config(
    ...
    if resolved_provider == "volcengine":
        resolved_api_key = api_key or os.getenv("ARK_API_KEY") or os.getenv("VOLCENGINE_API_KEY") or os.getenv("API_KEY")
        resolved_api_base_url = api_base_url or os.getenv("ARK_TRANSCRIPT_URL") or os.getenv("ARK_AUDIO_TRANSCRIPT_URL") or os.getenv("VOLCENGINE_TRANSCRIPT_URL") or f"{os.getenv('ARK_BASE_URL', os.getenv('VOLCENGINE_BASE_URL', DEFAULT_ARK_BASE_URL)).rstrip('/')}/audio/transcriptions"
        ...
        resolved_api_key = api_key or os.getenv("DASHSCOPE_API_KEY") or os.getenv("API_KEY")
        resolved_api_base_url = api_base_url or os.getenv("DASHSCOPE_TRANSCRIPT_URL") or DEFAULT_API_BASE_URL
        resolved_model = model or os.getenv("DASHSCOPE_ASR_MODEL") or os.getenv("DASHSCOPE_MODEL") or DEFAULT_MODEL
...
def extract_text(
    ...
    """从视频中提取文案并保存到文件"""
    config = resolve_asr_config(provider=provider, api_key=api_key, api_base_url=api_base_url, model=model)
    ...
    if show_progress:
        print(f"正在使用 {config['provider']} 从音频中提取文本...")
    text_content = processor.extract_text_from_audio(audio_path, show_progress=show_progress)

    result = {"video_info": video_info, "text": text_content, "output_path": None, "provider": config["provider"], "model": config["model"]}
    ...
    if output_dir:
        ...
        with open(transcript_path, "w", encoding="utf-8") as f:
            ...
            f.write(f"| 识别提供方 | `{config['provider']}` |\n")
            f.write(f"| 识别模型 | `{config['model']}` |\n")
...
示例:
  ...
  # 使用 DashScope 提取文案
  python douyin_downloader.py --link "抖音分享链接" --action extract --provider dashscope --output ./output

  # 使用火山引擎 / 方舟提取文案
  python douyin_downloader.py --link "抖音分享链接" --action extract --provider volcengine --output ./output
        """,
    )

    parser.add_argument("--link", "-l", required=True, help="抖音分享链接或包含链接的文本")
    parser.add_argument("--action", "-a", choices=["info", "download", "extract"], default="info", help="操作类型")
    parser.add_argument("--output", "-o", default="./output", help="输出目录 (默认 ./output)")
    parser.add_argument("--api-key", "-k", help="ASR API 密钥")
    parser.add_argument("--provider", choices=["dashscope", "volcengine"], help="ASR 提供方")
    parser.add_argument("--api-base-url", help="自定义 ASR 接口地址")
    parser.add_argument("--model", help="自定义 ASR 模型")
